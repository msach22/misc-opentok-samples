...
(function(window) {
  ...
  OT.properties = {
    ...
    cdnURL: 'http://localhost:8000/opentok',
...
/**
 * @license  Common JS Helpers on OpenTok 0.2.0 24638ec master
 * http://www.tokbox.com/
 *
 * Copyright (c) 2014 TokBox, Inc.
 *
...
!(function(window) {
  ...
  OT.PeerConnection = function(config) {
    ...
    var _collectedAudioOutputLevel = 0;
    ...
    // Private methods
    /**
     * Adapts the invocation to the native getStats implementation. It is required because browsers
     * do not implement the same version of the method.
     *
     * It memorize the detection to avoid re executing the test at every call.
     *
     * @param {function(number)} callback
     * @private
     */
    var _getStats = function(callback) {
      if (_getStats._treeParametersVersion) {
        _peerConnection.getStats(null, callback, function(error) {
          console.error(error);
        });
      } else {
        try {
          ...
          if (e instanceof TypeError) {
            _getStats._treeParametersVersion = true;
            _getStats(callback);
          } else {
            // rethrow the exception if accidentally caught
            throw e;
          }
        }
      }
    ...
    /**
     * Starts the collection of the audio output level. It updates "_collectedAudioOutputLevel"
     * 15 times per second.
     *
     * It is useful because Firefox does not (yet delivers) "audioOuputLevel" statistic but luckily
     * we can analyse the remote audio stream to get the audio level. This function should be run
     * only if the browser is capable of analysing remote audio streams.
     *
     * @param {MediaStream} stream
     * @returns {{cancel: function()}}
     */
    var startAudioOutputLevelCollection = function(stream) {
      var samplingFrequency = 10;
      var sourceNode = _audioContext.createMediaStreamSource(stream);
      var analyser = _audioContext.createAnalyser();
      var timeDomainData = new Uint8Array(analyser.frequencyBinCount);

      sourceNode.connect(analyser);

      var intervalId = setInterval(function sampleAudioOutputLevel() {

        analyser.getByteTimeDomainData(timeDomainData);

        var max = 0;
        for (var idx = 0; idx < timeDomainData.length; idx++) {
          max = Math.max(max, Math.abs(timeDomainData[idx] - 127));
        }

        // normalize the collected level to match the range delivered by
        // the getStats' audioOutputLevel
        _collectedAudioOutputLevel = max * OT.PeerConnection.AUDIO_OUTPUT_LEVEL_MAX_VALUE / 255;
    ...
    /**
     * Retrieves the PeerConnection's audioOutputLevel ratio value.
     *
     * The value is included in the range [0, 32767] (0 meaning no output)
     *
     * @memberof PeerConnection
     * @param {function(number)} callback
     */
    this.getAudioOutputLevel = function(callback) {
      var _this = this;

      _getStats(function(statsReport) {
        if(_this.getAudioOutputLevel._noAudioOutputLevelInStats) {
          callback(_collectedAudioOutputLevel);
        } else {
          var valueFound = false;

          // adapt the way different browsers represent stats report
          var results = statsReport.result ? statsReport.result() :
            Object.keys(statsReport).map(function(key) {return statsReport[key];});

          for (var i = 0; i < results.length; i++) {
            var result = results[i];
            if (result.local) {
              var audioOutputLevel = parseFloat(result.local.stat('audioOutputLevel'));
              if (!isNaN(audioOutputLevel)) {
                callback(audioOutputLevel);
                valueFound = true;
                break;
              }
            }
          ...
          if(!valueFound) {
            // if the value has not been found it means this browser doesn't deliver
            // audioOutputLevel in the stats report so we need to use the stream analysis
            // based strategy
            _this.getAudioOutputLevel._noAudioOutputLevelInStats = true;
            startAudioOutputLevelCollection(_peerConnection.getRemoteStreams()[0]);
            _this.getAudioOutputLevel(callback);
          }
        }
      });
    };
  ...
  /**
   * The maximum value of the audioOutputLevel statistic.
   *
   * This value has been guessed by reading the WebRTC library's source.
   *
   * @type {number}
   */
  OT.PeerConnection.AUDIO_OUTPUT_LEVEL_MAX_VALUE = 32767;
...
!(function() {
  ...
  OT.SubscriberPeerConnection = function(remoteConnection, session, stream,
    ...
    this.getAudioOutputLevel = function(callback) {
      _peerConnection.getAudioOutputLevel(callback);
    };
  ...
  OT.Subscriber = function(targetElement, options) {
    ...
    var logAnalyticsEvent = function(action, variation, payloadType, payload) {
        ...
        _createChrome = function() {
          ...
          (function startAudioOutputLevelSampling() {

            var averageAudioOutputLevel = null;

            (function sampleAudioOutputLevel() {
              _peerConnection.getAudioOutputLevel(function(outputLevel) {
                _audioOutputLevel = outputLevel;

                if (audioMeter) {
                  if (averageAudioOutputLevel === null) {
                    averageAudioOutputLevel = outputLevel;
                  } else {
                    averageAudioOutputLevel = outputLevel * 0.1 + averageAudioOutputLevel * 0.9;
                  }

                  if (averageAudioOutputLevel < 700) {
                    // noise threshold and stability check for log
                    audioMeter.value = 0;
                  } else {
                    audioMeter.value = ((Math.log(averageAudioOutputLevel /
                      OT.PeerConnection.AUDIO_OUTPUT_LEVEL_MAX_VALUE) / Math.LN10)) / 1.5 + 1;
                  }
                }
              });

              setTimeout(sampleAudioOutputLevel, 20);
            })();
          })();

          audioMeter = new OT.Chrome.Meter({
    ...
    */
    ...
    Object.defineProperties(this, {
      ...
      audioOutputLevel: {
        get: function() { return _audioOutputLevel; },
        enumerable: true
      }
    });
 ...
 */
  OT.Session = function(apiKey, sessionId) {
    ...
    var _thisSession = this;
    var _activities = {};
    window.setInterval(function() {

      var subscribers = OT.subscribers.where(function() {
        return true;
      });
      ...
      for (var idx = 0; idx < subscribers.length; idx++) {
        ...
        if (subscriber.audioOutputLevel > 0.2 * OT.PeerConnection.AUDIO_OUTPUT_LEVEL_MAX_VALUE) {
          if (streamId in _activities) {
            if (!_activities[streamId].talking) {
              if (Date.now() - _activities[streamId].timestamp > 2500) {
                // detected audio activity for more than 500ms for the first time
                _activities[streamId].talking = true;
                startedToTalk.push(_activities[streamId].subscriber);
              }
            } else {
              _activities[streamId].timestamp = Date.now();
            }
          } else {
            _activities[streamId] = {subscriber: subscriber, timestamp: Date.now(), talking: false};
          }
        } else {
          if (streamId in _activities) {
            var activity = _activities[streamId];
            if (Date.now() - activity.timestamp > 5000) {
              // didn't detect audio activity for more than 1s
              stoppedToTalk.push(activity.subscriber);
              delete _activities[streamId];
            }
          }
        }

        if (startedToTalk.length) {
          _thisSession.trigger('startedToTalk', {subscribers: startedToTalk});
        }

        if (stoppedToTalk.length) {
          _thisSession.trigger('stoppedToTalk', {subscribers: stoppedToTalk});
        }
      }
    }, 1000 / 10);
